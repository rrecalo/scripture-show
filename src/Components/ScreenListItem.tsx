...
function ScreenListItem({s, isActive, toggleScreen, renameCustomScreen, setIsEditing, setScreenToDelete} : Props){
  ...
  return (
    ...
    className="w-full py-2 ps-2 pe-3 border border-neutral-700 rounded-md text-neutral-200 text-sm flex justify-between items-center">
        ...
        <div className='flex justify-start gap-1 items-center align-middle w-full text-neutral-300'>
            {!editing ?
            <>
            {s.customName}
            <motion.div className="ml-2 mr-1 p-1 border border-neutral-700 rounded-md " onClick={()=>startEditing()} animate={{opacity: hover ? 1 : 0, display: hover ? 'block' : 'none'}}>
            <MdOutlineEdit className/>
            </motion.div>
            <motion.div className="p-1 border border-neutral-700 rounded-md" onClick={()=>openDeleteModal()} animate={{opacity: hover ? 1 : 0, display: hover ? 'block' : 'none'}}>
                <MdOutlineDelete className/>
            </motion.div>
            </>
            :
            <motion.input onKeyDown={handleKeyDown as any} className="text-neutral-100 outline-none bg-transparent" initial={{opacity:0}} animate={{opacity:1}} id="screen_name_edit" value={nameInput} onChange={(e)=>setNameInput(e.target.value.trimStart())} autoCapitalize='off' autoComplete='off' autoCorrect='off'/>
            }
        </div>
