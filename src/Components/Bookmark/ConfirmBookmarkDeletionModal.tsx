...
export default function ConfirmBookmarkDeletionModal({display, setDisplay, b, deleteBookmark} : BookmarkModalProps){
    ...
    return (
        <motion.div initial={{opacity:0}} animate={{opacity: display ? 1 : 0, display: display ? "block" : "none"}} exit={{opacity:0}} className={`p-5 absolute z-10 w-100 h-fit dark:bg-neutral-800
